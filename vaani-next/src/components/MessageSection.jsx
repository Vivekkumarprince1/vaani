...
const MessageSection = ({
    ...
    const { t, translateText, currentLanguage } = useTranslation();
    ...
    useEffect(() => {
        ...
        const translateVisibleMessages = async () => {
            const messagesToTranslate = messages.filter(msg => {
                ...
                return visibleMessageIds.has(messageId) && !translatedMessages.has(messageId);
            });

            if (messagesToTranslate.length === 0) return;

            // console.log(`🔄 Translating ${messagesToTranslate.length} visible messages to ${currentLanguage}`);

            // Translate messages in small batches to avoid overwhelming the API
            const batchSize = 5;
            for (let i = 0; i < messagesToTranslate.length; i += batchSize) {
                ...
                await Promise.all(
                    batch.map(async (msg) => {
                        const messageId = msg._id || msg.id;
                        try {
                            // Skip if already translated or if content is already in target language
                            if (translatedMessages.has(messageId)) return;

                            const translatedContent = await translateText(msg.content, currentLanguage);
                ...
                if (i + batchSize < messagesToTranslate.length) {
                    await new Promise(resolve => setTimeout(resolve, 100));
                }
            }

            // console.log(`✅ Translation completed for ${messagesToTranslate.length} messages`);
        };

        translateVisibleMessages();
    }, [messages, visibleMessageIds, currentLanguage, translateText, translatedMessages]);
            ...
            <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
                {messages.map((msg, index) => {
                    ...
                    return (
                            ...
                            >
                                <div className="break-words text-[15px]">{displayContent}</div>
